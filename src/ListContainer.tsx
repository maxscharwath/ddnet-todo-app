  ...
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <button className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-gray-100 hover:dark:bg-gray-700 active:bg-gray-100 data-[state=open]:bg-gray-100 data-[state=open]:dark:bg-gray-700">
          {icon}
        </button>
      </DropdownMenu.Trigger>
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          className="rounded p-5 bg-white dark:bg-gray-800 shadow-lg ring-1 ring-black ring-opacity-5 flex flex-col gap-4"
          sideOffset={5}
        >
          <div className="grid grid-cols-5 gap-2">
            {emojis.map(emoji => (
              <DropdownMenu.Item key={emoji} className='cursor-pointer w-8 h-8 flex items-center justify-center rounded-xl bg-gray-100 dark:bg-gray-700' onClick={() => onUpdateIcon?.(emoji)}>
                {emoji}
              </DropdownMenu.Item>
            ))}
          </div>
...
const ListTitle: FC<{
  ...
  return (
    <div className={clsx(
      "flex items-center rounded-xl h-8 px-2 hover:bg-gray-100 hover:dark:bg-gray-700 font-semibold text-lg",
  ...
  return (
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
          <Dialog.Title className="text-lg font-semibold text-gray-800 dark:text-gray-200">Share List</Dialog.Title>
          <Dialog.Description className="text-sm text-gray-400">Share this list with others and collaborate.</Dialog.Description>
          <div className="flex flex-col gap-4 mt-4">
            <ul className="flex flex-col gap-2 mt-4">
              {allowedClientKeys.map(key => (
                <li className="flex h-8"
                    key={key}>
                  <span className="flex-grow bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-200 px-2 rounded-md rounded-r-none text-sm font-medium flex items-center">{key}</span>
                  <button className="p-2 rounded-md rounded-l-none bg-red-500 text-white font-semibold disabled:opacity-50"
            ...
            <div className="flex h-8">
              <input className="flex-grow bg-gray-100 dark:bg-gray-700 px-2 rounded-md rounded-r-none text-sm font-medium text-gray-600 dark:text-gray-200"
                     type="text"
          ...
          <div className="flex gap-4 mt-4 justify-end">
            <Dialog.Close asChild>
              <button className="px-4 h-10 rounded-md bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-200 font-semibold mt-4">Cancel</button>
            </Dialog.Close>
...
export const AddUserIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" {...props}>
    <path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11h3m0 0h3m-3 0v3m0-3V8m-3 11v-1.25c0-2.071-1.919-3.75-4.286-3.75H7.286C4.919 14 3 15.679 3 17.75V19m9-11a3 3 0 1 1-6 0a3 3 0 0 1 6 0z"></path>
  </svg>
)

export const RemoveUserIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" {...props}>
    <path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 11h6m-6 8v-1.25c0-2.071-1.919-3.75-4.286-3.75H8.286C5.919 14 4 15.679 4 17.75V19m9-11a3 3 0 1 1-6 0a3 3 0 0 1 6 0z"></path>
  </svg>
)
...
export const ListContainer: FC<{ list: ListDocument }> = ({ list }) => {
  ...
  return (
    <div
      className="flex flex-col w-full h-full p-8 bg-white dark:bg-gray-800 rounded-lg shadow-lg text-gray-600 dark:text-gray-200 gap-4">
